// DOM-dependent sanitizeHtmlToFragment tests. jsdom (devDependency only — index.js
// imports nothing) is installed on globalThis BEFORE the kit is imported so
// sanitizeHtmlToFragment resolves document/DOMParser at call time, mirroring a browser.
...
const { sanitizeHtmlToFragment } = await import('../index.js');
...
test('keeps allowlisted tags and drops blocked subtrees entirely', () => {
  const out = htmlOf(sanitizeHtmlToFragment('<p>keep</p><script>alert(1)</script><style>p{}</style>'));
  assert.equal(out, '<p>keep</p>');
});

test('unwraps unknown tags but keeps their cleaned children', () => {
  const out = htmlOf(sanitizeHtmlToFragment('<section><article><p>prose</p> tail</article></section>'));
  assert.equal(out, '<p>prose</p> tail');
});

test('blocked subtree removal beats unwrapping (script inside div)', () => {
  const out = htmlOf(sanitizeHtmlToFragment('<div><script>alert(1)</script><em>ok</em></div>'));
  assert.equal(out, '<div><em>ok</em></div>');
});

test('drops event handlers and non-allowlisted attributes', () => {
  const out = htmlOf(sanitizeHtmlToFragment('<p onclick="alert(1)" class="x" data-y="z">t</p>'));
  assert.equal(out, '<p>t</p>');
});

test('keeps safe hrefs, decorates real links, drops dangerous hrefs', () => {
  const safe = htmlOf(sanitizeHtmlToFragment('<a href="https://example.com/a">x</a>'));
  assert.match(safe, /href="https:\/\/example\.com\/a"/);
  assert.match(safe, /target="_blank"/);
  assert.match(safe, /rel="noopener noreferrer"/);

  const unsafe = htmlOf(sanitizeHtmlToFragment('<a href="javascript:alert(1)">x</a>'));
...
test('img with unsafe or missing src is unwrapped by default', () => {
  assert.equal(htmlOf(sanitizeHtmlToFragment('<img src="javascript:alert(1)">')), '');
  assert.equal(htmlOf(sanitizeHtmlToFragment('<img>')), '');
  const kept = htmlOf(sanitizeHtmlToFragment('<img src="https://example.com/x.png">'));
...
test('lazyImages option stamps loading="lazy"', () => {
  const out = htmlOf(sanitizeHtmlToFragment('<img src="https://e.com/a.jpg">', { lazyImages: true }));
  assert.match(out, /loading="lazy"/);
});

test('globalAttrs allows benign attrs everywhere but never URL/event names', () => {
  const out = htmlOf(sanitizeHtmlToFragment('<p dir="rtl" lang="ar" onclick="x()" href="/a">t</p>', {
...
test('foreign content (svg) is dropped with its subtree, not unwrapped', () => {
  ...
  const out = htmlOf(sanitizeHtmlToFragment('<p>a</p><svg><style>&lt;img src=x onerror=alert(1)&gt;</style></svg>'));
...
test('a normalizing safeUrl validator can rewrite hrefs', () => {
  const out = htmlOf(sanitizeHtmlToFragment('<a href="/rel">x</a>', {
